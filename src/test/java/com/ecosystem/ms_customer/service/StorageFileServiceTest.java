...
public class StorageFileServiceTest {
    ...
    @Test
    @DisplayName("Should be possible remove file from aws s3")
    void should_be_possible_remove_file_from_aws_s3() {
        this.service.remove(Mockito.anyString());

        Mockito.verify(
                this.s3Client, Mockito.times(1)
        ).deleteObject(Mockito.any(DeleteObjectRequest.class));
    }
}
